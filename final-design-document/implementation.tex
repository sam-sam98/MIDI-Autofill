...
manner. During our sprint meetings, we look over the kanban board and decide which issues
from the backlog to work on next.
...
keeping the main branch stable. Everytime the code is in a stable state, ready for a new
version, the \url{develop} branch will be merged into the \url{main} branch.

Initially we used the Git Flow approach, where each developer codes features and bug fixes
on a separate branch, and only merges when the feature is entirely complete. We found this
approach to be too burdensome for us to collaborate with, as feature branches only get
merged in when they are completed. Often times we found ourselves needing to use code from
another branch that was not available on the main branch. We believe this approach will be
ideal after the project is mostly completed and is in a maintenance state.

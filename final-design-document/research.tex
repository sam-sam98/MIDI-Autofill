...
\nameref{sec:embedded_controllers}}, the Raspberry Pi is mini-computer which can also
...
All benchmarks in this suite use random notes quantized to quarter steps. These range of
these random notes is tailored to each checkpoint it is run against. For example, the
\url{basic_rnn} checkpoint only supports pitches in the range \url{[48, 84]}, while
\url{melody_rnn} supports the full MIDI range of \url{[0, 127]}.
...
verify that a Raspberry Pi can even run a browser based DAW with acceptable performance.
...
Raspberry Pi OS. The performance was acceptable, with a couple things feeling slow.
